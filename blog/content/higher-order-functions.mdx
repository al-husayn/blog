...
<Warning>
**Prerequisites:** This guide assumes you understand [scope and closures](/blog/scope-and-closures). Closures are created when higher-order functions return other functions. You should also be familiar with [callbacks](/blog/callbacks), since callbacks are the functions being passed to higher-order functions.
</Warning>
...
JavaScript provides many built-in higher-order functions, especially for working with arrays. These are covered in depth in the [Map, Reduce, and Filter](/blog/map-reduce-filter) guide, but here's a quick overview:
...
<Note>
For a deep dive into these methods with practical examples, see [Map, Reduce, and Filter](/blog/map-reduce-filter).
</Note>
...
<CardGroup cols={2}>
  <Card title="Callbacks" icon="phone" href="/blog/callbacks">
    Callbacks are functions passed to higher-order functions
  </Card>
  <Card title="Map, Reduce, Filter" icon="filter" href="/blog/map-reduce-filter">
  ...
  <Card title="Currying & Composition" icon="layer-group" href="/blog/currying-composition">
    Advanced patterns built on top of higher-order functions
  </Card>
  <Card title="Scope and Closures" icon="lock" href="/blog/scope-and-closures">
