...
The ```retryDelay``` option controls the spacing, and the default is I love the function form, because it lets you decide based on why the request failed. Retrying a ```404``` might be pointless (unless you have a race condition), since the resource doesn’t exist, and asking three more times won’t change that.
